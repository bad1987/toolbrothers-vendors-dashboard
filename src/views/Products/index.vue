...
import { ref, onMounted, onBeforeMount, onUpdated, computed } from "vue";
...
async function updateProduct(obj = null) {
  ...
  axios
    .put(
      "/products/" + selectedProduct.value.product_id,
      obj == null ? selectedProduct.value : obj
    )
    .then((response) => {
      let ans = products.value.map((x) =>
        x.product_id === selectedProduct.value.product_id ? response.data : x
      );
      products.value = ans;
...
function deactivateProduct(id) {
  Object.assign(
    ...
    products.value.find((x) => x.product_id == id)
  );

  updateProduct({
    status: selectedProduct.value.status == "A" ? "H" : "A",
...
function changeManually(id) {
  ...
  updateProduct({
    manual_change: selectedProduct.value.manual_change == true ? false : true,
    product_id: id,
  }).catch(() => {});
}
...
function fastForward() {
  actualSkip.value =
    (Math.ceil(totalProducts.value / actuaLimit.value) - 1) * actuaLimit.value;
  ...
  >
    ...
    >
      ...
      <div class="items-center justify-between lg:flex">
        ...
        <div class="items-center sm:flex my-auto">
              ...
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                ...
                <tbody class="bg-white dark:bg-gray-800">
                  ...
                  <tr v-for="product in products" :key="product.product_id">
                    ...
                    <td
                      class="p-4 cursor-pointer !w-32 line-clamp-1 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400"
                    >
                      <span
                        :data-tooltip-target="`tooltip-right-${product.product_id}`"
                        data-tooltip-placement="right"
                        class="w-full block overflow-ellipsis"
                        >{{ product.product }}</span
                      >
                      <div
                        :id="`tooltip-right-${product.product_id}`"
                        role="tooltip"
                    ...
                    <td
                      class="p-4 text-sm text-gray-900 whitespace-nowrap dark:text-white"
                    >
                    ...
                    <td class="">
                      <label class="relative inline-flex items-center cursor-pointer">
                        <input
                          v-model="product.manual_change"
                          type="checkbox"
                          value=""
                          class="sr-only peer"
                        />
                        <div
                          @click="changeManually(product.product_id)"
