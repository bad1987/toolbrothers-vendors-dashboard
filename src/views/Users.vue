...
import { ref, onMounted, onBeforeMount, onUpdated, watch, computed } from "vue";
...
const filteredUsers = computed(() => {
  return users.value.filter(
    (user) =>
      user.email.includes(searchTerm.value.toLowerCase()) ||
...
const fetchUsers = () => {
  ...
  userApi.fetchUsers(users, permissions, router, route, true, platforms)
    .finally(() => {
...
function changeSelectedUser(email) {
  Object.assign(
    selectedUser.value,
    users.value.find((x) => x.email == email)
  );
...
function login_by_vendor(vendor_id) {
  axios
    .get("/login_as_vendor/" + vendor_id)
    .then((response) => {
      window.open("/", "_blank");
    })
    .then(() => initFlowbite())
...
function changeSelectedStatus() {
  selectedUser.value.status =
    selectedUser.value.status == true
      ? "D"
      : selectedUser.value.status == "A"
        ? "D"
        : "A";
}

function changeSelectedPlatform(value) {
  selectedUser.value.platform_id = value != "" ? value : null;
}
...
const deleteUser = (user_id) => {
  axios
    ...
    .then((res) => {
      alert.value.showAlert("success", "User deleted successfully!!", "Successful!!");
      fetchUsers();
    })
    .catch((error) => {
      alert.value.showAlert("error", "Operation fail please retry !!", "error!!");
    });
};

// Send password to user
const sendpassword = (user_id) => {
  loadStore.changeLoadingStatus(true);
  axios
    .post("/admin/users/" + user_id + "/send-password")
    .then((res) => {
      alert.value.showAlert("success", "Password sent successfully!!", "Successful!!");
    })
    .catch((err) => {
      alert.value.showAlert("error", err.response.data.detail, "error!!");
    })
...
<template>
  ...
  <main class="mt-7 dark:bg-gray-800 dark:border-gray-700">
      ...
      <div class="w-full mb-1">
        ...
        <div class="sm:flex">
          <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700">
            <form class="lg:pr-3" action="#" method="GET">
              ...
              <div class="relative mt-1 lg:w-64 xl:w-96">
                <input v-model="searchTerm" autocomplete="off" type="text" name="email" id="users-search"
          ...
          <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
            <button @click="selectedPermissions = []" type="button"
              class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
              data-te-toggle="modal" data-te-target="#add-user-modal" data-te-ripple-init data-te-ripple-color="light">
            ...
            <button v-if="!isImporting && route.params.type === 'vendors'" data-modal-toggle="import-modal"
              data-modal-target="import-modal" @click="launchUpload"
            ...
            <svg v-if="isImporting && route.params.type === 'vendors'" class="loading w-7 h-7" fill="none"
              stroke="#75ebeb" stroke-width="2.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true">
              ...
              <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700 relative">
                ...
                <tr v-for="(u, idx) in filteredUsers" :key="idx" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                  ...
                  <td class="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
                    <img class="w-10 h-10 rounded-full" src="../assets/images/user-bg.png"
                  ...
                  <td class="p-4 space-x-2 whitespace-nowrap">
                    <button @click="changeSelectedUser(u.email)" type="button"
                      ...
                      data-te-toggle="modal" data-te-target="#edit-user-modal" data-te-ripple-init
                    ...
                    <button @click="deleteUser(u.id)" type="button"
                      class="inline-flex items-center px-3 py-1 text-sm font-medium text-center text-white bg-red-400 rounded-lg hover:bg-red-600 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                    ...
                    <button v-if="route.params.type == 'vendors'" @click="sendpassword(u.id)" type="button"
                      class="inline-flex items-center px-3 py-1 text-sm font-medium text-center text-white bg-gray-400 rounded-lg hover:bg-gray-600 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-900">
                      <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <button @click="login_by_vendor(u.id)" type="button"
                      class="inline-flex items-center px-3 py-1 text-sm font-medium text-center text-white bg-green-400 rounded-lg hover:bg-green-600 focus:ring-4 focus:ring-green-300 dark:focus:ring-green-900">
                      <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div data-te-modal-init
      ...
      id="edit-user-modal" tabindex="-1" aria-labelledby="edit-user-modalTitle" aria-modal="true" role="dialog">
      <div data-te-modal-dialog-ref
        ...
        <div
          ...
          <div
            ...
            <!--Close button-->
            <button type="button" id="edit-user-close"
              ...
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3" v-if="route.params.type == 'vendors'">
                  <label for="role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a
                    role</label>
                  <select @change="changeRole" id="role"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="">Select a role</option>
                    <option :selected="selectedUser.roles == 'Role_affiliate'" value="Role_affiliate">
                      Affiliate
                    </option>
                    <option :selected="selectedUser.roles == 'Role_direct_sale'" value="Role_direct_sale">
                      Direct sale
                    </option>
                  </select>
                </div>
                <div class="mt-9 flex">
                  <input @change="changeSelectedStatus" :checked="selectedUser.status == 'A'"
                    id="checkbox-activate-create" type="checkbox" value=""
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                  <label for="checkbox-activate-create"
                ...
                <div class="col-span-6">
                  ...
                  <div class="permissions-list">
                    <CheckboxGroup v-if="permissions.length" :selected="selectedUser.permissions" :items="permissions"
                      :is-grouped="true" name="permission" id="checkbox-group-perm"
                      @toggle-value="togglePermissionValue" />
                  </div>
                </div>
                <div class="col-span-6" v-if="route.params.type == 'vendors' && platforms.length > 0">
                  <label for="role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a
                    platform</label>
                  <select data-te-select-init @change="changeSelectedPlatform($event.target.value)" id="platform"
                    ...
                    <option :value="platform.id" v-for="platform in platforms"
                      :selected="platform.id == selectedUser.platform_id">{{ platform.name }}</option>
                  </select>
                </div>
          ...
          <div class="items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-700">
            <ButtonComponent @click="updateUser(null)" text="Save all"
    ...
    <div data-te-modal-init
      class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
      id="add-user-modal" tabindex="-1" aria-labelledby="add-user-modalTitle" aria-modal="true" role="dialog">
      <div data-te-modal-dialog-ref
        class="pointer-events-none relative flex min-h-[calc(100%-1rem)] w-auto translate-y-[-50px] items-center opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:min-h-[calc(100%-3.5rem)] min-[576px]:max-w-[500px]">
        <div
          class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none dark:bg-neutral-600">
          ...
          <div
            ...
            <!--Close button-->
            <button type="button" id="add-user-close"
              class="box-content rounded-none border-none hover:no-underline hover:opacity-75 focus:opacity-100 focus:shadow-none focus:outline-none"
              data-te-modal-dismiss aria-label="Close">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="h-6 w-6">
            ...
            <form action="#" id="add-user-form">
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3">
                  <label for="role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a
                    role</label>
                  <select v-model="newUser.roles" @change="changeRole" id="role"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div class="mt-9 flex">
                  <input v-model="newUser.status" id="checkbox-activate-create-vendor" type="checkbox" value=""
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                  <label for="checkbox-activate-create-vendor"
                    class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Activate</label>
                </div>
                <div class="col-span-6">
                  <h4 class="font-bold dark:text-white">Set permissions</h4>
                  <div class="permissions-list">
                    <CheckboxGroup v-if="permissions.length" :is-grouped="true" :items="permissions" name="permission"
                      id="checkbox-group-perm" @toggle-value="togglePermissionValue" />
                  </div>
                </div>
              </div>
            </form>
            ...
            <form action="#" id="add-admin-form">
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="flex items-center mb-4">
                  <input v-model="newUser.status" id="checkbox-activate-create-admin" type="checkbox" value=""
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                  <label for="checkbox-activate-create-admin"
                    class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Activate</label>
                </div>
                <div class="col-span-6">
                  <h4 class="font-bold dark:text-white">Set permissions</h4>
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div class="p-6 text-center">
            ...
            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
              Are you sure you want to start importing vendors?
            </h3>
            <button @click="startUpload" data-modal-hide="import-modal" type="button"
