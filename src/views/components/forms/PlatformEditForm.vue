<script setup>
import { ref, watch, onMounted, defineProps, onUpdated, computed } from 'vue'
...
const languages = computed(() => {
    return new Set(props.selectedPlatform.map(platform => platform.language).sort())
})

function pushField() {
    // verify if a field is empty in any of the platforms
    let emptyField = false
    props.selectedPlatform.forEach(platform => {
...
<template>
    <div data-te-modal-init
        ...
        id="edit-platform-modal" tabindex="-1" aria-labelledby="edit-platform-modalTitle" aria-modal="true" role="dialog">
            ...
            <div
                ...
                <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
                    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="myTab"
                        data-tabs-toggle="#myTabContent-update" role="tablist">
                        <li v-for="(language, idx) in languages" :key="idx" class="mr-2" role="presentation">
                            <button class="inline-block p-4 border-b-2 rounded-t-lg" :id="language + '-tab-update'"
                                :data-tabs-target="'#platform-' + language + '-update'" type="button" role="tab" :aria-controls="language"
                                :aria-selected="idx == 0">{{ language.toUpperCase() }}</button>
                        </li>
                    </ul>
                </div>
                <div id="myTabContent-update">
                    <div v-for="(platform, idx) in selectedPlatform" :key="idx" class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" :id="'platform-' + platform.language + '-update'" role="tabpanel"
                        :aria-labelledby="platform.language + '-tab-update'">
                        <div class="p-4">
                            <div class="mb-6">
                            ...
                            <div class="mt-9 flex">
                                <input v-model="platform.status" id="checkbox-activate-create-platform"
                                    type="checkbox" value=""
                ...
                <div class="items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-700">
                    <ButtonComponent @click="updatePlatform()" :text="$t('save_platform')"
