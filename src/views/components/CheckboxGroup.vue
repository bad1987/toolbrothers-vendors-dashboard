...
<template>
  <div
    ...
    :key="idx"
    :class="isHorizontal ? 'grid grid-cols-3' : ''"
  >
    <input
    ...
    />
    <label
      :for="`checkbox-${props.name}-${idx}`"
      class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
      >{{ item.description ?? item.text }}</label
    >
  </div>
  <div v-else>
    <div class="grid grid-cols-2 gap-4">
      <div class="" v-for="group in itemsByGroup" :key="group.name">
        <div class="my-3">
          <strong>{{ group.name }} {{ $t("mb_permissions") }}: </strong>
        </div>
        <div class="" v-for="(item, idx) in group.items" :key="idx">
          <input
            :checked="isChecked(item.value)"
            @change="toggleValue(item)"
            :id="`checkbox-${props.name}${group.name}-${idx}`"
            type="checkbox"
            :value="item.value"
            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
          />
          <label
            :for="`checkbox-${props.name}${group.name}-${idx}`"
            class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
