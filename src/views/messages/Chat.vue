...
const getChatByThread = () => {
  axios
    ...
    .then((response) => {
      chat.value = response.data.datas;
      skeletonCnt.value = 0;
      isSuccess.value = true


      if (response.data.status == 'N') markAsRead(response.data.status)
...
function markAsRead(val) {
  axios.put('/messages/' + thread_id + '/change-status').finally(() => {
    status.value = val == 'N' ? 'V' : (val == 'V' ? 'N' : val)
  })
}
...
<template>
  <div class="mx-auto">
    ...
    <div class="m-auto border rounded dark:border-gray-600">
      <div class="">
        <div class="w-full ">
          <div class="flex justify-between items-center border-b border-gray-300 dark:border-gray-600">
            <div class="relative flex items-center p-3">
              <svg class="object-cover w-10 h-10 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor"
              ...
              <span class="absolute w-3 h-3 bg-green-600 rounded-full left-10 top-3" v-if="status == 'N'">
              </span>
              <span class="absolute w-3 h-3 bg-gray-600 rounded-full left-10 top-3" v-if="status == 'V'">
              </span>
              <span class="absolute w-3 h-3 bg-red-600 rounded-full left-10 top-3" v-if="status == 'D'">
              </span>
            </div>
            <div class="mr-5 flex items-center space-x-3">
              <span class="font-bold" v-if="isUnRead">Mark as read</span>
              <span class="font-bold" v-if="!isUnRead">Mark as unread</span>
              ...
              <div role="status" class="animate-pulse" v-if="!isSuccess">
                <div class="" v-for="p = 0 in 10" :key="p">
                  <div class="flex items-center">
                    <svg class="w-10 h-32 mr-2 text-gray-200 dark:text-gray-700" aria-hidden="true" fill="currentColor"
                ...
                <div v-for="item in chat" :key="item.message_id">
                  <div class="flex mb-4 message" v-if="item.user_type != 'V'">
                    <div class="flex-2">
                      <div class="relative w-12 h-12">
                        <svg class="object-cover w-10 h-10 text-gray-600 dark:text-gray-400" fill="none"
                    ...
                    <div class="flex-1 max-w-2xl px-2">
                      ...
                      <div class="pl-4"><small class="text-gray-500 text-[10px]"><span class="font-semibold">{{
                        item.cscart_users.lastname }} {{ item.cscart_users.firstname }}</span> {{ item.timestamp }}
                        </small></div>
                    </div>
                  </div>
                  <div class="flex mb-4 text-right message me " v-if="item.user_type == 'V'">
                    <div class="flex-1 px-2 ">
                      ...
                      <div class="pr-4"><small class="text-gray-500 text-[10px]"><span class="font-semibold"> {{
                        item.cscart_users.lastname }} {{ item.cscart_users.firstname }}</span>
                          {{ item.timestamp }}</small></div>
                    </div>
                  </div>
                </div>
                <div id="end"></div>
          ...
          <form @submit.prevent="handlerSubmit($event)"
            class="flex items-center justify-between w-full p-3 border-t border-gray-300 dark:border-gray-600">
            <button>
            ...
            <input type="text" placeholder="Message"
              ...
              name="message" v-model="input" required />
