...
class OrderUsecase:
    ...
    def get_order_list(self, request: Request, db_local: Session, db_cscart: Session, skip: int, limit: int, status: str, start_time: datetime, end_time: datetime) -> OrderResponseModel:
        return self.order_repository.get_orders(request=request,db_local=db_local,db_cscart=db_cscart,skip=skip, limit=limit, status=status, start_time=start_time, end_time=end_time)
