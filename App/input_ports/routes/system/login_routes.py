...
async def login_post(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    response.set_cookie(Settings.COOKIE_NAME, res[Settings.COOKIE_NAME], domain='localhost', path='/', max_age=max_age, samesite='Strict', secure=secure_cookie)
...
def logout(request: Request, response: Response):
    response.delete_cookie(Settings.COOKIE_NAME, domain='localhost', path='/', samesite='Strict', secure=secure_cookie)
