...
s_user_route = APIRouter(prefix='/sys/admin', tags=['Users system'], include_in_schema=False)
...
async def get_users_list(
    ...
    user_usecase: UserUsecase = Depends(get_db)
):
...
def get_user(request: Request, db: Session = Depends(get_db), user_usecase: UserUsecase = Depends(get_db)):
